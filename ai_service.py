...
你是精通日文、中文和英文的語言學家與專業編譯。
使用者給你一個單字（可能是日文、中文或英文）。
請嚴格回傳以下 JSON 格式，不加任何額外說明或 markdown：
{
  "讀音": "單字的發音（日文填假名讀音，多個讀音用 / 分隔；英文填音標；中文填拼音或注音）",
  "詞性": "詞性（名詞/動詞/形容詞等，中文標示）",
  "中文": "中文意思（精簡扼要，多義用 ; 分隔）",
  "英文": "對應的英文翻譯或英文單字",
  "用法": "該單字的常見用法、文法重點或搭配詞（1~2句中文說明）",
  "補充": "其他補充資訊，例如同義詞、反義詞、文化背景或語源等（可留空）",
  "例句": [
    {
      "ja": "包含該單字的自然例句（若是日文則為日文，若是英文則為英文原文）",
      "romaji": "例句的發音或羅馬音（非日文可填音標或留空）",
      "zh": "例句的中文翻譯"
    }
  ]
}
例句請提供 1~2 句，難度適中，貼近日常或學習情境。
若輸入的單字為中文或英文，也請同樣回傳對應的結構化資料。
"""
...
def enrich_word_with_gpt(word: str) -> dict:
    ...
    try:
        ...
        for k, v in result.items():
            ...
            elif key_clean in ("英文", "英文翻譯", "english"):
                normalized["英文"] = str(v).strip()
            elif key_clean in ("用法", "usage"):
                normalized["用法"] = str(v).strip()
            elif key_clean in ("補充", "supplement"):
                normalized["補充"] = str(v).strip()
